...
def eaAdaptiveWithoutHall(population, toolbox, ngen, k1, k2, k3, k4, stats=None, verbose=__debug__):
	def getMinAvg(record):
		return record['min'], record['avg']
...
def eaSimpleWithoutHall(population, toolbox, cxpb, mutpb, ngen, stats=None, verbose=__debug__):
	# Create a logbook
	logbook = tools.Logbook()
	logbook.header = ['gen', 'nevals'] + (stats.fields if stats else [])
